...
public class NewsServiceImpl implements NewsService {
    ...
    public NewsListResponse listar(String category, Integer limit) {
        ...
        if (category != null && !category.isBlank()) {
            ...
            List<NewsItem> itens =
                    newsRepository.findActiveByCategory(category, now, Limit.of(limiteEfetivo));
            return newsMapper.toListResponse(itens);
        }

        List<NewsItem> itens = newsRepository.findActiveExcludingCategory(
                EDUCATION_READING_CATEGORY, now, Limit.of(limiteEfetivo));
        return newsMapper.toListResponse(itens);
    }
    ...
    public NewsArticleResponse buscarPorSlug(String slug) {
        ...
        if (!item.isVisibleAt(Instant.now())) {
            throw new ResourceNotFoundException("Artigo não encontrado: " + slug);
        }
    ...
    public NewsListResponse listarComSlug(Integer limit) {
        ...
        List<NewsItem> itens = newsRepository.findActiveWithSlugExcludingCategory(
                EDUCATION_READING_CATEGORY, Instant.now(), Limit.of(limiteEfetivo));
        return newsMapper.toListResponse(itens);
    }
